...
class ViewAssegnaDieta(object):
    objMetodi = Metodi()
    peso = 0
    percorso_file_dieta_personali = "./Dieta/Data/FilesDieteCliente/"
    ...
    def invia_file_dieta(self):
        percorso = self.controller.allega_file_dieta()
        if percorso != "":
            estensione = percorso.split(".")
            indice = len(estensione)
            self.controller.rimuovi_dieta_assegnata(self.username, self.percorso_file_dieta_personali)
            shutil.copy(percorso, self.percorso_file_dieta_personali + self.username + "." + estensione[indice - 1])
    ...
    def popola_dati_fisiologici(self):
        info_dieta = self.controller.get_info_dieta(self.username)
        if info_dieta != 0:
            self.txtBmi.setText(info_dieta.bmi)
            self.txtPesoIdeale.setText(info_dieta.peso_ideale)
            self.txtFabbisogno.setText(info_dieta.fabbisogno)
            self.plainTextEdit.setPlainText(info_dieta.segnalazioni)
        else:
    ...
    def popola_mostra_suggerimenti(self):
        self.peso = self.controller.get_peso(self.username)
        if self.peso != 0:
    ...
    def setupUi(self, MainWindow):
        ...
        self.nascondi()
        self.rbtnSuggerimenti.clicked.connect(self.mostra)
        ...
        self.btnCalcolaNutrienti.clicked.connect(self.calcola_fabbisogno)
